...
module LucidIntercom
  RSpec.shared_examples 'event' do
    it { is_expected.to have_attributes(company: instance_of(CompanyAttributes)) }
    it { is_expected.to have_attributes(company_custom: instance_of(CompanyCustomAttributes)) }
    it { is_expected.to have_attributes(user: instance_of(UserAttributes)) }
